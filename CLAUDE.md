...
npx softuq remove button     # delete component files + clear from softuq.json
npx softuq list              # show available components (installed marked with ✓)
npx softuq list --installed  # show only installed components
npx softuq diff              # show which components have upstream updates
npx softuq update            # pull updated components (with confirmation)
npx softuq doctor            # diagnose project setup (alias, provider, CSS order, etc.)
npx softuq skill             # install design skill to .claude/skills/ (project)
npx softuq skill -g          # install globally to ~/.claude/skills/
```

- `init` detects framework (React only today; Svelte detected but exits with a "not yet available" message) and refuses to run on unsupported frameworks instead of crashing mid-scaffold
...
- `init` writes `softuq.json` with `{ framework, componentDir, libDir, components: {} }`. The `components` map tracks installed components as `{ [name]: { installedAt } }` — added on `init` (starter set) + `add`, removed by `remove`. Backward compatible: older configs without the field default to `{}` on read.
- `add` resolves transitive registry deps (e.g. select → tag) and records them in `softuq.json`
- `add` rewrites import paths (`../../lib/utils` → `@/lib/utils`)
- `remove` deletes component files + clears them from `softuq.json`; reverse-dep check refuses to drop a component another installed one still requires (`--force` to override, `--yes` to skip confirm). npm dependencies stay in `package.json` — manual cleanup if unused.
...
- `doctor` does read-only diagnostics: `@/*` alias in tsconfig, Vite Tailwind plugin + alias, CSS import order, softuq-tokens / softuq-theme imports, `SoftuqProvider` in layout/main, `data-theme` attr, fonts wired. Each check prints `✓/!/✗` with a fix-hint. Exits non-zero on fails. No mutations.
- `installDeps` runs package managers with `stdio: "inherit"` (streams output live) + try/catch → exits 1 with a helpful message on failure, no more silent crashes when `pnpm add` hits a peer conflict or network error
- `softuq --version` reads from `package.json` via `createRequire` — never hardcode in `src/index.ts`
- Registry: `packages/cli/src/registry/react.json` — maps components to files, npm deps, internal deps. Bundled into the CLI tarball **and** synced to `softuq.com/registry.json` during docs build (see "Remote registry" below).
- Shared config helpers: `packages/cli/src/utils/config.ts` — `readConfig` / `writeConfig` / `markInstalled` / `markUninstalled`. Always use these, never read/write `softuq.json` ad-hoc from commands.
- File reads: use `readSource(framework, relPath)` and `readTokenFile(relPath)` from `utils/registry.ts` instead of `fs.readFile(path.join(sourceDir, ...))`. These prefer bundled templates (fast, offline) and fall back to `softuq.com/r/...` when a file is missing — that's how new components ship without a CLI bump.
- Getting started tutorial: `/getting-started` page in docs app

### Remote registry

The docs site mirrors the CLI registry at `softuq.com/registry.json` and component/token source files at `softuq.com/r/{react,tokens}/...`. Generated by `packages/docs/scripts/sync-registry.mjs`, which runs via `predev` and `prebuild` in `packages/docs/package.json`. Railway auto-deploy on push to main propagates the updated registry without republishing the CLI.

- CLI side: `packages/cli/src/utils/remote.ts` — `fetchRemoteManifest` (cached 1 h in `~/.softuq/cache/registry.json`, 3 s timeout) + `fetchRemoteFile`.
- Override base URL via `SOFTUQ_REGISTRY_URL` env (e.g. `SOFTUQ_REGISTRY_URL=http://localhost:3333` when testing against local docs).
- Bundled fallback: if remote fetch + cache both fail (offline, docs outage), commands still work against the templates baked into the CLI tarball — `add` can install any component that was in the registry at CLI publish time.
- Generated output lives at `packages/docs/public/registry.json` + `packages/docs/public/r/` — both gitignored, recreated every `predev`/`prebuild`.
- Updating the registry schema or the sync script requires coordination: bump the shared shape in `packages/cli/src/utils/registry.ts` (`Registry` interface) + `packages/docs/scripts/sync-registry.mjs` in the same PR.
