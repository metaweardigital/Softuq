        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-[var(--ds-space-stack)]">
          ...
          {COLUMNS.map((col) => (
            <div key={col.title} className="space-y-[var(--ds-space-gap)]">
              <h3 className="text-xs font-semibold tracking-wide uppercase text-fg-dimmed">{col.title}</h3>
              <ul className="space-y-2">
                {col.links.map((l) => (
                  <li key={l.label}>
                    <a href={l.href} className="text-sm text-fg-secondary hover:text-accent transition-colors">
                      {l.label}
                    </a>
                  </li>
                ))}
