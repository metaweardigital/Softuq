...
function ComponentSidebar() {
  const grouped = groupByLetter(COMPONENT_SECTIONS);
  return (
    <SectionNav
      observeSelector="section[id]"
      className="sticky top-20 flex max-h-[calc(100vh-6rem)] overflow-y-auto overflow-x-visible scrollbar-hidden"
    >
      {grouped.map(([letter, items]) => (
        <SectionNavGroup key={letter} label={letter} firstSectionId={items[0].toLowerCase().replace(/\s+/g, "-")}>
          {items.map((name) => {
            const id = name.toLowerCase().replace(/\s+/g, "-");
            return (
              <SectionNavItem key={id} href={`#${id}`}>
                <span className="hidden md:inline">{name}</span>
                <span className="md:hidden size-1.5 rounded-full bg-current" />
              </SectionNavItem>
            );
          })}
        </SectionNavGroup>
          ...
          <div className="min-w-0 flex-1 space-y-12">
                ...
                <div className="space-y-2">
                  ...
                  <Accordion variant="bordered" type="multiple">
                    ...
                    <AccordionItem value="b3">
                      ...
                      <AccordionContent>
                        Yes. Every component ships with full <span className="text-accent">TypeScript types</span> —
                        props, variants, and refs are fully inferred.
                      </AccordionContent>
                    </AccordionItem>
                  </Accordion>
                </div>
            ...
            <Section title="Alert">
              <div className="space-y-3">
                ...
                <p className="text-xs font-medium text-fg-muted uppercase tracking-wide pt-4">
...
export default function Page() {
  ...
  <Button>Generate</Button>
            ...
            <Section title="Section Nav">
              <Row label="Alphabetical groups">
                <SectionNav activeId="button" className="w-40">
                  <SectionNavGroup label="A">
                    <SectionNavItem sectionId="accordion">Accordion</SectionNavItem>
                    <SectionNavItem sectionId="alert">Alert</SectionNavItem>
                  </SectionNavGroup>
                  <SectionNavGroup label="B">
                    <SectionNavItem sectionId="badge">Badge</SectionNavItem>
                    <SectionNavItem sectionId="button">Button</SectionNavItem>
                  </SectionNavGroup>
                  <SectionNavGroup label="C">
                    <SectionNavItem sectionId="card">Card</SectionNavItem>
                  </SectionNavGroup>
                </SectionNav>
              </Row>
            </Section>
